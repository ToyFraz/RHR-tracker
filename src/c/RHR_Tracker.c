...
static void update_history() {
  time_t today = time_start_of_today();
  if (s_app_state.current_week_start == 0) s_app_state.current_week_start = today;
  ...
  if (today > s_app_state.last_recorded_day) {
    ...
    if (days_passed > 0) {
      int shift = (days_passed > MAX_DAYS) ? MAX_DAYS : days_passed;
      for (int i = MAX_DAYS - 1; i >= shift; i--) {
        s_app_state.daily_history[i] = s_app_state.daily_history[i - shift];
      }
      for (int i = 0; i < shift; i++) s_app_state.daily_history[i] = NEUTRAL_HR; 
    }
    ...
    while (today >= s_app_state.current_week_start + (7 * SECONDS_PER_DAY)) {
      if (s_app_state.current_week_days_counted > 0) {
        ...
        for (int i = MAX_WEEKS - 1; i > 0; i--) s_app_state.weekly_averages[i] = s_app_state.weekly_averages[i - 1];
...
static void select_click_handler(ClickRecognizerRef recognizer, void *context) {
  ...
  text_layer_set_text(s_label_text_layer, s_showing_14_days ? "14 Day History" : "120 Day History");
...
static void graph_update_proc(Layer *layer, GContext *ctx) {
  ...
  graphics_draw_line(ctx, GPoint(10, bounds.size.h - 10), GPoint(bounds.size.w - 10, bounds.size.h - 10));
  graphics_draw_line(ctx, GPoint(10, 10), GPoint(10, bounds.size.h - 10));

  uint8_t latest_hr = s_app_state.daily_history[0] ? s_app_state.daily_history[0] : NEUTRAL_HR;
  int graph_min_hr = latest_hr - (GRAPH_SPREAD / 2);
  int graph_max_hr = latest_hr + (GRAPH_SPREAD / 2);
  if (graph_min_hr < 0) graph_min_hr = 0;
  ...
  for (int i = 0; i < num_bars; i++) {
    ...
    if (val > 0) {
      int bar_height = ((val - graph_min_hr) * max_graph_height) / GRAPH_SPREAD;
      if (bar_height <= 0) bar_height = 1;
      if (bar_height > max_graph_height) bar_height = max_graph_height;

      int x_pos = 12 + (i * bar_width);
      int y_pos = bounds.size.h - 10 - bar_height;

      graphics_context_set_fill_color(ctx, (i == num_bars - 1) ? GColorDukeBlue : GColorPictonBlue); 
...
static void main_window_load(Window *window) {
  ...
  if (s_current_night_hr > 0) {
    ...
    snprintf(s_hr_buffer, sizeof(s_hr_buffer), "Calculating...");
  }
